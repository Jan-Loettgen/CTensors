...
__global__
void _vecAddKernal(float *A, float *B, float *C, int n){
...
int mat_add_C(tensor_2d* mat_a, tensor_2d* mat_b, tensor_2d* mat_out){
    ...
    //kernal invocation
    _vecAddKernal<<<ceil(mat_a->n_elems/NUM_THREADS_PER_BLOCK), NUM_THREADS_PER_BLOCK>>>(d_data_A, d_data_B, d_data_out, mat_a->n_elems);
